...
class NewsLinkScraper:
    ...
    def _find_links_by_text(self, soup: BeautifulSoup, search_text: str, 
        ...
        Returns:
            List of link tags containing the search text
        """
        search_container = container or soup
        
        # Direct text match
        links = search_container.find_all(
            'a', 
            string=lambda text: text and search_text.lower() in text.lower()
        )
        ...
        if not links:
            ...
            for link in all_links:
                if link.text and search_text.lower() in link.text.lower().strip():
                    links.append(link)
